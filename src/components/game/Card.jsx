          ...
          <div className="flex justify-between items-start">
            <div className="flex flex-col items-start w-[25%]">
                <div className="relative w-[100%] aspect-square">
                    <img src="/images/mana_icon.png" alt="mana" draggable={false} className="absolute inset-0 w-full h-full object-contain drop-shadow-md" onError={(e) => e.target.style.display = 'none'} />
                    <div className={`absolute inset-0 flex items-center justify-center font-black text-white pt-0 ${textShadow} z-10 ${isGameDetail ? 'text-4xl' : 'text-[15cqw]'}`}>{card.cost}</div>
                </div>
          ...
          <div className="flex flex-col gap-[2%]">
            {isGameDetail && card.description && (
                <div className="absolute top-[55%] left-0 w-full px-4 z-50">
                    <div className="bg-black/90 text-white text-lg p-2 rounded border border-white/30 shadow-2xl text-center leading-tight font-medium max-h-24 overflow-y-auto">{card.description}</div>
                </div>
            )}
            {!['library', 'detail', 'game-detail'].includes(location) && (
                <div className="bg-black/60 rounded px-1 py-[2%] backdrop-blur-[1px] mb-[2%]">
                  <div className={`text-white text-center font-bold truncate ${isGameDetail ? 'text-lg py-1' : 'text-[8cqw]'}`}>{card.name}</div>
                </div>
            )}
              ...
              {card.type === 'unit' && (
                <>
                  <div className="relative w-[24%] aspect-square group">
                    ...
                    <div className={`absolute inset-0 flex items-center justify-center font-black pt-[1cqw] ${textShadow} z-10 ${getAttackColor()} ${isGameDetail ? 'text-4xl pt-1' : 'text-[15cqw]'}`}>{card.attack}</div>
                  </div>
                  <div className="relative w-[24%] aspect-square group">
                    ...
                    <div className={`absolute inset-0 flex items-center justify-center font-black pt-[1cqw] ${textShadow} z-10 ${getCurrentHpColor()} ${isGameDetail ? 'text-4xl pt-1' : 'text-[15cqw]'}`}>{card.currentHp !== undefined ? card.currentHp : card.health}</div>
                  </div>
                </>
              )}
              {card.type === 'building' && (
                <div className={`w-full text-center font-bold text-slate-200 ${textShadow} ${isGameDetail ? 'text-xl' : 'text-[14cqw]'}`}>耐久 {card.currentHp !== undefined ? card.currentHp : card.health}</div>
              )}
              {card.type === 'spell' && (<div className={`w-full text-center text-cyan-300 font-bold drop-shadow-md ${isGameDetail ? 'text-base' : 'text-[8cqw]'}`}>SPELL</div>)}
            </div>
          </div>
        </div>
